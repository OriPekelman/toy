# Shared OpenAI-shape /v1/embeddings handler. Used by every
# tep_demo/openai_api_*.rb server.
#
# Wire pattern in each server:
#
...
#   policy; future PR can add tokenizer-side encode).
...
#   Tao's /v1/embeddings consumer typically wants one vector per
#   input. (For per-token output, a follow-up adds an `encoding`
#   knob or returns the matrix; out of scope for v1.)
