# Per-phase training step bench (CUDA). Mirrors the inference bench
# (demos/qwen25_phase_bench.rb in spirit) but for the LoRA training
# inner loop. Builds the training graph once via the same path as
# demos/smollm2_lora_train_ce_cuda.rb, then runs N steps with per-phase
# Time.now markers around graph_reset / uploads / compute_backward /
# download.
#
# Run: GGUF=data/smollm2-135m-native.gguf ./demos/smollm2_lora_train_bench_cuda
#
# Reports per-phase mean (ms), and percentage of total step time each
# phase consumes. Compare against the inference bench
# (docs/design/bench-cuda-2026-05-21.md) — the training step is heavier
# (forward+backward+540 opt_step nodes vs forward only) but uses the
# SAME compute graph re-realization model, so phase ratios should
# illuminate where M3 (reusable decode graph) buys us the most.
...
while s < N_STEPS
  ...
  loss_vals.push(TinyNNCuda.tnn_scratch_get(sess, 0))
  s = s + 1
end
...
puts ""
puts "=== Per-step phase means (CUDA, " + N_STEPS.to_s + " steps after " + N_WARMUP.to_s + " warmup) ==="
