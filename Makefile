...
examples/example_train_from_scratch_cpu: examples/06_train_from_scratch.rb lib/llama_seq_forward_ffi.rb lib/toy_smollm2.rb lib/transformer.rb lib/tinynn.rb tinynn/libtinynn_ggml.a
	$(SPINEL) $< -o $@
examples/example_train_from_scratch: examples/example_train_from_scratch_cpu
	@printf '#!/bin/sh\n# Auto-generated by Makefile. DEVICE selects the backend binary.\n# Edit examples/06_train_from_scratch.rb (cpu) for behaviour.\ncase "$${DEVICE:-cpu}" in\n  cpu|"") exec "$$(dirname "$$0")/example_train_from_scratch_cpu" "$$@" ;;\n  cuda)   echo "DEVICE=cuda not yet supported for from-scratch training" >&2; echo "(LlamaSeqForwardFFICacheCuda lacks realize_for_random_init; see toy#train-device-select-cuda)" >&2; exit 2 ;;\n  metal)  echo "DEVICE=metal not yet supported for training (inference only)" >&2; exit 2 ;;\n  *)      echo "DEVICE=$${DEVICE} not recognised (cpu only for now)" >&2; exit 2 ;;\nesac\n' > $@
	@chmod +x $@
