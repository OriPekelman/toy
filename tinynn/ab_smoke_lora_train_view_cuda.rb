...
t_W_q  = TinyNNCuda.tnn_input_2d_f32_persistent(sess, K, K)
...
t_K_cache = TinyNNCuda.tnn_input_2d_f32_persistent(sess, MAX_T, K)   # (K, MAX_T)
t_V_cache = TinyNNCuda.tnn_input_2d_f32_persistent(sess, K, MAX_T)   # (MAX_T, K)
...
bytes_max_T  = MAX_T * 4
t_K_view = TinyNNCuda.tnn_view_2d(sess, t_K_cache, K, T_CTX, bytes_d_head, 0)
t_V_view = TinyNNCuda.tnn_view_2d(sess, t_V_cache, T_CTX, K, bytes_max_T, 0)

scale_v = 1.0 / Math.sqrt(K.to_f)
t_scores = TinyNNCuda.tnn_matmul(sess, t_K_view, t_q)
t_scaled = TinyNNCuda.tnn_scale(sess, t_scores, scale_v)
t_attn   = TinyNNCuda.tnn_softmax(sess, t_scaled)
t_head   = TinyNNCuda.tnn_matmul(sess, t_V_view, t_attn)
...
TinyNNCuda.upload_row_major(sess, t_K_cache, m_K)
m_V = Mat.new(K, MAX_T); i = 0
while i < K * MAX_T; m_V.flat[i] = next_normal(0.3); i = i + 1; end
TinyNNCuda.upload_row_major(sess, t_V_cache, m_V)
