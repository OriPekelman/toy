# Task #70 bisect step 2: add rope_ext to the LoRA toy chain.
#
# Forward:  y = (W + B@A) @ x;  yr = rope_ext(y, pos);  loss = sum(yr²)
# Note: y is shape (OUT=8, 1) — rope_ext treats it as (d_head=8, T=1).
# K=8 with pos=[0] gives rope frequencies for a single token.
...
t_yr    = TinyNNCuda.tnn_rope_ext(sess, t_y, t_pos, OUT, 10000.0)
t_yr_sq = TinyNNCuda.tnn_mul(sess, t_yr, t_yr)
t_loss  = TinyNNCuda.tnn_sum(sess, t_yr_sq)
