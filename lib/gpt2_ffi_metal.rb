...
class GPT2FullForwardFFICacheMetal
  ...
  def initialize
    ...
    @t_pos_slice   = TinyNNMetal.tnn_null_ptr
    @t_token_ids   = TinyNNMetal.tnn_null_ptr
    @t_ln_f_gamma  = TinyNNMetal.tnn_null_ptr
    @t_ln_f_beta   = TinyNNMetal.tnn_null_ptr
    @t_x_embed     = TinyNNMetal.tnn_null_ptr
    @t_x_final     = TinyNNMetal.tnn_null_ptr
    @t_logits      = TinyNNMetal.tnn_null_ptr
    @gpt2_blocks_ffi = [GPT2BlockFFIMetal.new]
  end
  ...
  def realize_for(t_seq, d_model, d_ff, n_heads, n_layers, vocab_size)
    ...
    @t_pos_slice   = TinyNNMetal.tnn_input_2d_f32_persistent(@sess, t_seq,      d_model)
    ...
    # Per-block handles — seed-then-push so Spinel types as Array<GPT2BlockFFIMetal>.
    @gpt2_blocks_ffi = [GPT2BlockFFIMetal.new]
    li = 1
    while li < n_layers
      @gpt2_blocks_ffi.push(GPT2BlockFFIMetal.new)
      li = li + 1
    end
    ...
    while li < n_layers
      ...
      blk.t_b_v = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, 1)]
      h = 1
      while h < n_heads
    ...
    # === Compute input ===
    @t_token_ids = TinyNNMetal.tnn_input_1d_i32(@sess, t_seq)

    # === Forward graph ===
    # x_embed = token_embed[ids] + pos_slice  (ne=[d_model, T])
    t_embedded = TinyNNMetal.tnn_get_rows(@sess, @t_token_embed, @t_token_ids)
    @t_x_embed = TinyNNMetal.tnn_add(@sess, t_embedded, @t_pos_slice)
    TinyNNMetal.tnn_set_output(@t_x_embed)
    ...
    # Final LayerNorm.
    @t_x_final = TinyNNMetal.tnn_layer_norm(@sess, t_cur, @t_ln_f_gamma, @t_ln_f_beta, eps)
    TinyNNMetal.tnn_set_output(@t_x_final)

    # Tied unembed: logits = mul_mat(token_embed, x_final)  ne=[vocab, T]
    @t_logits = TinyNNMetal.tnn_matmul(@sess, @t_token_embed, @t_x_final)
    TinyNNMetal.tnn_set_output(@t_logits)

    TinyNNMetal.tnn_realize(@sess, @t_logits)
    @realized = true
  end
  ...
  def build_block(t_x, blk, eps, scale)
    t_h1 = TinyNNMetal.tnn_layer_norm(@sess, t_x, blk.t_ln1_gamma, blk.t_ln1_beta, eps)
    ...
    t_pre_raw= TinyNNMetal.tnn_matmul(@sess, blk.t_w_ff1, t_h2)
    t_pre    = TinyNNMetal.tnn_add(@sess, t_pre_raw, blk.t_b_ff1)
    t_hidden = TinyNNMetal.tnn_gelu(@sess, t_pre)
    t_dn_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_ff2, t_hidden)
  ...
  def build_attention_head(t_x, t_w_q, t_w_k, t_w_v, t_b_q, t_b_k, t_b_v, scale)
    t_q_raw = TinyNNMetal.tnn_matmul(@sess, t_w_q, t_x)        # ne=[d_head, T]
    t_q     = TinyNNMetal.tnn_add(@sess, t_q_raw, t_b_q)
    t_k_raw = TinyNNMetal.tnn_matmul(@sess, t_w_k, t_x)
    t_k     = TinyNNMetal.tnn_add(@sess, t_k_raw, t_b_k)
    # v in transposed pattern (ne=[T, d_head]) so head_out's k_dim matches.
    t_v_raw = TinyNNMetal.tnn_matmul(@sess, t_x, t_w_v)
    t_v     = TinyNNMetal.tnn_add(@sess, t_v_raw, t_b_v)

    t_scores = TinyNNMetal.tnn_matmul(@sess, t_k, t_q)
    ...
    TinyNNMetal.tnn_matmul(@sess, t_v, t_attn)                  # ne=[d_head, T_query]
  end
end

module GPT2FFIMetal
  # Upload all weights from a populated GPT2LM into a freshly-realized
  # GPT2FullForwardFFICacheMetal. Transposed-upload for the per-head Q/K/V
