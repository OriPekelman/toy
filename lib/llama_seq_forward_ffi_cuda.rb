...
class LlamaSeqForwardFFICacheCuda
  ...
  def realize_for_random_init(cfg, t_seq, t_batch, weight_dtype, untied, qkv_bias, seed, init_scale)
    ...
    if @seq_donor_d_in > 0
      ...
      TinyNNCuda.tnn_tensor_set_name(@t_seq_token_embed, "token_embd.weight")
      # W_proj ne=[donor_d_in, d_model] so matmul(W_proj, embed)
      # contracts donor_d_in → d_model. Trainable.
      @t_seq_w_proj = TinyNNCuda.tnn_input_2d_f32_persistent(@sess,
  ...
  def build_forward_in_current_ctx
    ...
    if @seq_donor_d_in > 0
      t_proj = TinyNNCuda.tnn_matmul(@sess, @t_seq_w_proj, @t_seq_x_embed)
      TinyNNCuda.tnn_set_output(t_proj)
      t_cur = t_proj
    else
      t_cur = @t_seq_x_embed
    end
  ...
  def mp_matmul(w, x)
    if @seq_weight_dtype != 0
      wc = TinyNNCuda.tnn_cast(@sess, w, @seq_weight_dtype)
  ...
  def build_seq_block(t_x, blk, scale, eps)
    ...
    while hkv < @seq_n_kv
      ...
      # no-op (1 == 1).
      tb = @seq_t * @seq_b
      t_k_pre3 = TinyNNCuda.tnn_reshape_3d(@sess, t_k_pre, @seq_d_head, 1, tb)
      ...
      t_k      = TinyNNCuda.tnn_reshape_2d(@sess, t_k3, @seq_d_head, tb)
      t_k_per_kv.push(t_k)
  ...
  def build_seq_qhead(t_h, blk, hq, t_k_per_kv, t_vt_per_kv, scale)
    ...
    tb = @seq_t * @seq_b
    t_q_pre3 = TinyNNCuda.tnn_reshape_3d(@sess, t_q_pre, @seq_d_head, 1, tb)
    t_q3     = TinyNNCuda.tnn_rope_ext(@sess, t_q_pre3, @t_seq_positions,
                                     ...
                                     @seq_rope_scaling.beta_slow,
                                     @t_seq_rope_freq_factors)
    t_q      = TinyNNCuda.tnn_reshape_2d(@sess, t_q3, @seq_d_head, tb)
    ...
    if @seq_b > 1
      ...
      t_attn = TinyNNCuda.tnn_soft_max_ext(@sess, t_scores, @t_seq_attn_mask, scale, 0.0)
    else
