# lib/toy/llm/recipes/from_scratch_metal.rb — Metal twin of
...
# Toy::LLM::Engine::LlamaSeqEngineMetal (the Metal cache) and computes via TinyNNMetal.
# Numerics / op-order are IDENTICAL to the CPU recipe; only the cache type
# (line: @fs_cache) and the compute module prefix (TinyNN. -> TinyNNMetal.)
# differ.
#
# This file does NOT `require_relative "tinynn_metal"`: the loading monolith
# (lib/llama_seq_forward_ffi_metal.rb, required by the Metal runner before this
# file) already loads TinyNNMetal, exactly like the CPU recipe relies on
# llama_seq_forward_ffi.rb loading TinyNN. The Metal monolith transitively
...
module Toy; module LLM; module Recipes
  ...
  class FromScratchMetal
    ...
    def step!(seq_ids, positions, m_labels, m_hp, is_first)
      ...
      TinyNNMetal.upload_int_array(s, @fs_cache.t_seq_token_ids, seq_ids)
      TinyNNMetal.upload_int_array(s, @fs_cache.t_seq_positions, positions)
      TinyNNMetal.upload_row_major(s, @fs_t_labels, m_labels)
      TinyNNMetal.upload_row_major(s, @fs_t_hp,     m_hp)
      TinyNNMetal.tnn_compute_backward(s)
      loss_mat = TinyNNMetal.download_row_major(s, @fs_t_loss, 1, 1)
      loss_mat.flat[0]
    end
  end
end; end; end
