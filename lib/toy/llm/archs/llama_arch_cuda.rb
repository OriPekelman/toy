# lib/toy/llm/archs/llama_arch_cuda.rb — CUDA mirror of lib/toy/llm/archs/llama_arch.rb.
#
...
# TinyNNCuda before requiring this arch, exactly like the L1 primitives and
...
module Toy; module LLM; module Archs
  ...
  class LlamaArch
    ...
    def initialize
      ...
      @seq_rope_cfg           = TinyNNCuda.tnn_null_ptr
    end
    ...
    def build_forward(sess, t_token_ids, t_positions, t_rope_freq_factors,
      ...
      x_embed = TinyNNCuda.tnn_get_rows(sess, self.t_seq_token_embed, t_token_ids)
      TinyNNCuda.tnn_set_output(x_embed)
      ...
      if self.seq_donor_d_in > 0
        t_proj = TinyNNCuda.tnn_matmul(sess, self.t_seq_w_proj, x_embed)
      ...
      if seq_has_untied_output
        logits = TinyNNCuda.tnn_matmul(sess, self.t_seq_output, x_final)
      else
        logits = TinyNNCuda.tnn_matmul(sess, self.t_seq_token_embed, x_final)
      end
      TinyNNCuda.tnn_set_output(logits)
