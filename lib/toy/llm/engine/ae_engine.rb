...
class AeEngine
  ...
  def build_training_step
    ...
    t_f = TinyNN.tnn_rms_norm(@sess, t_h, @ft_weights[nw - 4], 1.0e-5)
    ...
    @t_latent = TinyNN.tnn_matmul(@sess, @ft_weights[nw - 3], t_f)  # [d, T]
    ...
    @t_logits = TinyNN.tnn_add(@sess,
                  TinyNN.tnn_matmul(@sess, @ft_weights[nw - 2], t_probe),
                  @ft_weights[nw - 1])
  ...
  def upload_random_init!(seed, init_scale)
    ...
    while wi < @ft_weights.length
      ...
      nm = @ft_names[wi]
      if nm == "head_bias"
        # Starts at ZERO so step 1 is exactly the no-bias model; the
        # unigram prior is LEARNED, not handed over.
