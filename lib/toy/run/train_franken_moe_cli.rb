      ...
      module TrainFrankenMoe
        ...
        def self.feedback_sig(sess, bups, bdowns, bglus)
          ...
          while fk < 3
            ...
            fj = 0
            while fj < arr.length
              n = TinyNN.tnn_tensor_nelements(arr[fj])
              buf = zeros(n)
              TinyNN.tnn_download_to_f64_array(sess, arr[fj], buf, n)
              k = 0
              while k < n
                acc = acc + buf[k] * buf[k]
                k = k + 1
        ...
        def self.feedback_cos_head(sess, b_last, t_emb, vocab, dm)
          nb = TinyNN.tnn_tensor_nelements(b_last)
          if nb != vocab * dm
            return 0.0
          end
          bb = zeros(nb)
          TinyNN.tnn_download_to_f64_array(sess, b_last, bb, nb)
          ee = zeros(nb)
          TinyNN.tnn_download_to_f64_array(sess, t_emb, ee, nb)
        ...
        def self.run_cli
          ...
          if lrc_s.length > 0 && lrc_s != "none" && lrc_s != "reactive"
            puts "toy-train-franken-moe: FRANKEN_LR_CONTROL " + lrc_s + " unsupported (none|reactive)"
            return 1
          end
          ...
          if !ctrl_on && (lrcw_s.length > 0 || lrcp_s.length > 0 || lrcf_s.length > 0 ||
            ...
            puts "toy-train-franken-moe: --lr-control-window/-patience/-factor/-recover/-floor need --lr-control reactive"
            return 1
          end
          ...
          if ctrl_on
            if ctrl_window < 1
              puts "toy-train-franken-moe: --lr-control-window must be >= 1"
              return 1
            end
            if ctrl_patience < 1
              puts "toy-train-franken-moe: --lr-control-patience must be >= 1"
              return 1
            end
            if ctrl_factor <= 0.0 || ctrl_factor >= 1.0
              puts "toy-train-franken-moe: --lr-control-factor must be in (0, 1) — it is a CUT"
              return 1
            end
            if ctrl_recover < 1.0
              puts "toy-train-franken-moe: --lr-control-recover must be >= 1.0 — it restores toward 1.0"
              return 1
            end
            if ctrl_floor <= 0.0 || ctrl_floor > 1.0
              puts "toy-train-franken-moe: --lr-control-floor must be in (0, 1]"
              return 1
            end
          end
          ...
          if fb_s.length > 0 && fb_s != "fixed" && fb_s != "kolen-pollack"
            puts "toy-train-franken-moe: FRANKEN_DFA_FEEDBACK " + fb_s + " unsupported (fixed|kolen-pollack)"
            return 1
          end
          ...
          if kp_on && block_dfa
            puts "toy-train-franken-moe: --dfa-feedback kolen-pollack does not yet reach --dfa-granularity block (the block feedback matrix is not coupled; the arm would run and silently NOT adapt). Use the default matmul granularity."
            return 1
          end
          fbd_s = ENV["FRANKEN_DFA_FEEDBACK_DECAY"] || ""
          fbl_s = ENV["FRANKEN_DFA_FEEDBACK_LR"]    || ""
          if !kp_on && (fbd_s.length > 0 || fbl_s.length > 0)
            puts "toy-train-franken-moe: --dfa-feedback-decay/-lr need --dfa-feedback kolen-pollack"
            return 1
          end
          fb_decay = fbd_s.length > 0 ? fbd_s.to_f : 1.0e-3
          if kp_on && fb_decay < 0.0
            puts "toy-train-franken-moe: --dfa-feedback-decay must be >= 0"
            return 1
          end
          ...
          if kp_on && fb_lr <= 0.0
            puts "toy-train-franken-moe: --dfa-feedback-lr must be > 0 (a zero feedback LR is a FIXED-B run wearing the kolen-pollack label)"
            return 1
          end
          ...
          if kp_on
            ...
            while fp < b_ups.length
              TinyNN.tnn_set_param(b_ups[fp])
              TinyNN.tnn_set_param(b_downs[fp])
              TinyNN.tnn_set_param(b_glus[fp])
              fp = fp + 1
            end
          end
          sels = [np0]; sels.pop
          ...
          t_hp_fb = TinyNN.tnn_input_1d_f32_persistent(sess, 2)
          ...
          while s < steps
            ...
            if kp_on
              TinyNN.tnn_upload_from_float_array(sess, t_hp_fb, [fb_lr, fb_decay], 2)
            end
            if top1
          ...
          if ev_corpus.length > 0
            ...
            while evw < ev_want
              ...
              if opt_code != 0
                TinyNN.tnn_upload_from_float_array(sess, t_hp_sgd, ev_hp_sgd, 2)
              end
              if lr_ramp
                elp = 0
                while elp < n_layers
                  if opt_code != 2
                    TinyNN.tnn_upload_from_float_array(sess, tw.t_hps[elp], ev_hp, 7)
                  end
                  if opt_code != 0
                    TinyNN.tnn_upload_from_float_array(sess, tw.t_hps_sgd[elp], ev_hp_sgd, 2)
                  end
                  elp = elp + 1
                end
              end
