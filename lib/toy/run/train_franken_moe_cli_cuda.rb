    ...
    module Run
      module TrainFrankenMoe
        ...
        def self.run_cli
          ...
          if n_shared < 0
            puts "toy-train-franken-moe-cuda: FRANKEN_MOE_SHARED must be >= 0"
            return 1
          end
          ...
          if latent_on
            lat_w = dmv / 2
            if lat_w < 1
              puts "toy-train-franken-moe-cuda: --moe-latent needs d_model >= 2 (got " + dmv.to_s + ")"
              return 1
            end
          end
          ...
          if attn_gate && top1 && !bp_spine
            puts "toy-train-franken-moe-cuda: --attn-gate needs --moe-policy bp-spine under top1 (the fully-DFA/bp-router lanes have no tap for W_g's input — it would sit frozen)"
            return 1
          end
