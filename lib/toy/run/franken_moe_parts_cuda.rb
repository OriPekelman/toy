...
require_relative "../llm/primitives/muon_cuda"
require_relative "../llm/primitives/situ_glu_cuda"
    ...
    module Run
      module TrainFrankenMoe
        ...
        def self.apply_step(sess, tw, t_hp, t_hp_sgd, idx, grad)
          ...
          to = TinyNNCuda.tnn_null_ptr
          if optv == 1 && muon_eligible(idx)
            nel = TinyNNCuda.tnn_tensor_nelements(tw.pp[idx])
            ne0 = TinyNNCuda.tnn_tensor_ne0(tw.pp[idx])
            ne1 = nel / ne0
            ...
            to = TinyNNCuda.tnn_opt_step_sgd(sess, tw.pp[idx], step, t_hp_sgd)
          elsif optv == 2
            to = TinyNNCuda.tnn_opt_step_sgd(sess, tw.pp[idx], grad, t_hp_sgd)
          else
            to = TinyNNCuda.tnn_opt_step_adamw(sess, tw.pp[idx], grad, tw.pm[idx], tw.pv[idx], t_hp)
          end
        ...
        def self.donor_embed_values(donor_path, vocab, dm, seed)
          gg = TinyNNCuda.tnn_gguf_load(donor_path)
          if gg == nil || gg == TinyNNCuda.tnn_null_ptr
            puts "donor: cannot open " + donor_path
            return zeros(0)
          end
          idx = TinyNNCuda.tnn_gguf_find_index(gg, "token_embd.weight")
          if idx < 0
            puts "donor: " + donor_path + " has no token_embd.weight"
            return zeros(0)
          end
          d_donor = TinyNNCuda.tnn_gguf_tensor_ne(gg, idx, 0)
          v_donor = TinyNNCuda.tnn_gguf_tensor_ne(gg, idx, 1)
          if v_donor != vocab
          ...
          src = zeros(n_src)
          TinyNNCuda.tnn_gguf_read_f32_to_doubles(gg, idx, src, n_src)

          dsess = TinyNNCuda.tnn_session_new(0)
          TinyNNCuda.tnn_session_set_graph_capacity(dsess, 262144)
          t_src = TinyNNCuda.tnn_input_2d_f32_persistent(dsess, v_donor, d_donor)  # ne=[d_donor, V]
          t_p   = TinyNNCuda.tnn_input_2d_f32_persistent(dsess, dm, d_donor)       # ne=[d_donor, dm]
          TinyNNCuda.tnn_finalize_weights(dsess)
          TinyNNCuda.tnn_upload_from_float_array(dsess, t_src, src, n_src)
          ...
          TinyNNCuda.tnn_upload_from_float_array(dsess, t_p, pv, pn)
          t_out = TinyNNCuda.tnn_matmul(dsess, t_p, t_src)   # ne=[dm, V]
          TinyNNCuda.tnn_set_output(t_out)
          TinyNNCuda.tnn_build_forward_only(dsess, t_out)
          TinyNNCuda.tnn_compute(dsess)
          n_dst = dm * vocab
          dst = zeros(n_dst)
          TinyNNCuda.tnn_download_to_f64_array(dsess, t_out, dst, n_dst)
          TinyNNCuda.tnn_session_free(dsess)
        ...
        def self.attention_block(sess, t_x, rn, wq, wk, wv, wo, mask, wg)
          ...
          ctx_in = ctx
          if wg != TinyNNCuda.tnn_null_ptr
            g_att  = TinyNNCuda.tnn_sigmoid(sess, TinyNNCuda.tnn_matmul(sess, wg, t_x))
            ctx_in = TinyNNCuda.tnn_mul(sess, g_att, ctx)
          end
          out    = TinyNNCuda.tnn_matmul(sess, wo, ctx_in)
          [TinyNNCuda.tnn_add(sess, t_x, out), h, ctx_in]
        end
        ...
        def self.shared_experts(sess, tw, h2, l)
          ...
          sj = 0
          while sj < nsv
            a = TinyNNCuda.tnn_gelu(sess, TinyNNCuda.tnn_matmul(sess, tw.pp[shared_base(l) + 2 * sj], h2))
            o = TinyNNCuda.tnn_matmul(sess, tw.pp[shared_base(l) + 2 * sj + 1], a)
            if sj == 0
              acc = o
            else
        ...
        def self.latent_up(sess, tw, u, l)
          ...
          TinyNNCuda.tnn_matmul(sess, tw.pp[lat_base(l) + 1], un)
        end
        ...
        def self.moe_block(sess, tw, t_x, sels, blk_cut, l)
          ...
          z = h2
          if latv > 0
            z = TinyNNCuda.tnn_matmul(sess, tw.pp[lat_base(l) + 0], h2)   # [ℓ, T]
          end
          if blk_cut == 1
            z = TinyNNCuda.tnn_detach(sess, z)
          end
          tw.tap_z = z
          tw.tap_zs.push(z)
          acc = TinyNNCuda.tnn_null_ptr
          ...
          while ei < nev
            ...
            u_i = TinyNNCuda.tnn_matmul(sess, tw.pp[up_idx(l, ei)], z)              # [DFF,T]
            a_i = TinyNNCuda.tnn_gelu(sess, u_i)
            if eactv == 1
              ...
              a_i = Toy::LLM::Primitives::SiTUGLU.gate(sess,
                      TinyNNCuda.tnn_matmul(sess, tw.pp[eglu_base(l) + ei], z), u_i)
            end
            tw.tap_as.push(a_i)
            if ei == 0
              tw.tap_a1 = a_i
          ...
          if blk_cut == 1
            ...
            out = TinyNNCuda.tnn_detach(sess, routed)
          end
          if nsv > 0
            out = TinyNNCuda.tnn_add(sess, out, shared_experts(sess, tw, h2, l))
          end
        ...
        def self.moe_block_top1(sess, tw, t_x, eye, cut, qb_bias, l)
          ...
          tw.tap_h2s.push(h2)
          r_logits = TinyNNCuda.tnn_matmul(sess, wr, h2)              # [NE, T]
          TinyNNCuda.tnn_set_output(r_logits)
          ...
          sel_scores = r_logits
          if qbb != TinyNNCuda.tnn_null_ptr
            sel_scores = TinyNNCuda.tnn_add(sess, r_logits, qbb)  # broadcast [NE,T]+[NE,1]
          end
          ids   = TinyNNCuda.tnn_argmax(sess, sel_scores)             # I32 [T]
          ...
          ag = probs
          if l > 0
            h2a = Toy::LLM::Primitives::RMSNorm.build(sess, TinyNNCuda.tnn_detach(sess, t_x), rn2, EPS)
            ag  = TinyNNCuda.tnn_softmax(sess, TinyNNCuda.tnn_matmul(sess, wr, h2a))
          end
          tw.t_auxgates_l.push(TinyNNCuda.tnn_sum_rows(sess, TinyNNCuda.tnn_mul(sess, oneh, ag)))

          # toy#128: stack the E experts by chained concat along dim 2
          # (E=2 == the original single concat pair).
          ew = ewidth
          up_stack = TinyNNCuda.tnn_reshape_3d(sess, tw.pp[up_idx(l, 0)],  ew, dfv, 1)
          dn_stack = TinyNNCuda.tnn_reshape_3d(sess, tw.pp[down_idx(l, 0)], dfv, ew, 1)
          ei = 1
          while ei < nev
            up_stack = TinyNNCuda.tnn_concat(sess, up_stack,
              TinyNNCuda.tnn_reshape_3d(sess, tw.pp[up_idx(l, ei)],  ew, dfv, 1), 2)   # [ℓ,DFF,E]
            dn_stack = TinyNNCuda.tnn_concat(sess, dn_stack,
              TinyNNCuda.tnn_reshape_3d(sess, tw.pp[down_idx(l, ei)], dfv, ew, 1), 2)   # [DFF,ℓ,E]
            ei = ei + 1
          end
          ...
          z_t = h2
          if latv > 0
            z_t = TinyNNCuda.tnn_matmul(sess, tw.pp[lat_base(l) + 0], h2)
          end
          tw.tap_z = z_t
          tw.tap_zs.push(z_t)
          h_exp = z_t
          ...
          if eactv == 1
            ...
            gt_stack = TinyNNCuda.tnn_reshape_3d(sess, tw.pp[eglu_base(l)], ew, dfv, 1)
            gi2 = 1
            while gi2 < nev
              gt_stack = TinyNNCuda.tnn_concat(sess, gt_stack,
                TinyNNCuda.tnn_reshape_3d(sess, tw.pp[eglu_base(l) + gi2], ew, dfv, 1), 2)
              gi2 = gi2 + 1
            end
            gto = TinyNNCuda.tnn_mul_mat_id(sess, gt_stack, h3, ids2)     # [DFF,1,T]
          ...
          routed = latent_up(sess, tw, TinyNNCuda.tnn_mul(sess, eo, gate), l)
          if nsv > 0
            routed = TinyNNCuda.tnn_add(sess, routed, shared_experts(sess, tw, h2, l))
          end
        ...
        def self.kp_update(sess, b, e, a_out, vocab, d_out, t_hp_fb)
          e_t  = TinyNNCuda.tnn_cont_2d(sess, TinyNNCuda.tnn_transpose(sess, e), tv, vocab)
          ao_t = TinyNNCuda.tnn_cont_2d(sess, TinyNNCuda.tnn_transpose(sess, a_out), tv, d_out)
          gb   = TinyNNCuda.tnn_matmul(sess, e_t, ao_t)
          TinyNNCuda.tnn_set_output(gb)
          to = TinyNNCuda.tnn_opt_step_sgd(sess, b, gb, t_hp_fb)
          TinyNNCuda.tnn_extend_backward_graph(sess, to)
          0
        end
        ...
        def self.full_sum(sess, x, n_elem)
          col = TinyNNCuda.tnn_reshape_2d(sess, x, n_elem, 1)
          TinyNNCuda.tnn_sum_rows(sess, col)
        end
