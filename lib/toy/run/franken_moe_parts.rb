    ...
    module Run
      module TrainFrankenMoe
        ...
        def self.shared_experts(sess, tw, h2)
          ...
          sj = 0
          while sj < nsv
            a = TinyNN.tnn_gelu(sess, TinyNN.tnn_matmul(sess, tw.pp[shared_base + 2 * sj], h2))
            o = TinyNN.tnn_matmul(sess, tw.pp[shared_base + 2 * sj + 1], a)
            if sj == 0
              acc = o
            else
        ...
        def self.moe_block(sess, tw, t_x, sels, blk_cut)
          ...
          while ei < nev
            ...
            o_i = TinyNN.tnn_matmul(sess, tw.pp[10 + 2 * ei], a_i)   # [ℓ or DM, T]
        ...
        def self.moe_block_top1(sess, tw, t_x, eye, cut, qb_bias)
          ...
          ew = ewidth
          up_stack = TinyNN.tnn_reshape_3d(sess, tw.pp[9],  ew, dfv, 1)
          dn_stack = TinyNN.tnn_reshape_3d(sess, tw.pp[10], dfv, ew, 1)
          ei = 1
          while ei < nev
            up_stack = TinyNN.tnn_concat(sess, up_stack,
              TinyNN.tnn_reshape_3d(sess, tw.pp[9 + 2 * ei],  ew, dfv, 1), 2)   # [ℓ,DFF,E]
            dn_stack = TinyNN.tnn_concat(sess, dn_stack,
              TinyNN.tnn_reshape_3d(sess, tw.pp[10 + 2 * ei], dfv, ew, 1), 2)   # [DFF,ℓ,E]
            ei = ei + 1
          end
          ...
          z_t = h2
          if latv > 0
            z_t = TinyNN.tnn_matmul(sess, tw.pp[lat_base + 0], h2)
          end
          tw.tap_z = z_t
          h_exp = z_t
          if cut == 1
          ...
          routed = latent_up(sess, tw, TinyNN.tnn_mul(sess, eo, gate))
          if nsv > 0
            routed = TinyNN.tnn_add(sess, routed, shared_experts(sess, tw, h2))
          end
