# lib/toy_smollm2_ffi_kv_metal.rb — Metal mirror of lib/toy_smollm2_ffi_kv.rb.
#
...
class SmolLM2KVBlockFFIMetal
  ...
  def initialize
    @t_rn1_gamma = TinyNNMetal.tnn_null_ptr
    ...
    @t_b_q  = [TinyNNMetal.tnn_null_ptr]   # per-Q-head bias (Qwen2.x)
    @t_b_k  = [TinyNNMetal.tnn_null_ptr]   # per-KV-head bias
    @t_b_v  = [TinyNNMetal.tnn_null_ptr]   # per-KV-head bias (1-D [d_head])
    @t_K    = [TinyNNMetal.tnn_null_ptr]
    @t_V    = [TinyNNMetal.tnn_null_ptr]
    @t_w_o    = TinyNNMetal.tnn_null_ptr
    @t_w_gate = TinyNNMetal.tnn_null_ptr
    @t_w_up   = TinyNNMetal.tnn_null_ptr
    @t_w_down = TinyNNMetal.tnn_null_ptr
    @t_w_router    = TinyNNMetal.tnn_null_ptr
    @t_w_gate_exps = TinyNNMetal.tnn_null_ptr
    @t_w_up_exps   = TinyNNMetal.tnn_null_ptr
    @t_w_down_exps = TinyNNMetal.tnn_null_ptr
    @t_w_lora_a_q = [TinyNNMetal.tnn_null_ptr]
    @t_w_lora_b_q = [TinyNNMetal.tnn_null_ptr]
    @t_w_lora_a_q_m = [TinyNNMetal.tnn_null_ptr]
    @t_w_lora_a_q_v = [TinyNNMetal.tnn_null_ptr]
    @t_w_lora_b_q_m = [TinyNNMetal.tnn_null_ptr]
    @t_w_lora_b_q_v = [TinyNNMetal.tnn_null_ptr]
  end
end
...
class SmolLM2KVFFICacheMetal
  ...
  def realize_for_mmap(gguf_handle, cfg, max_T, untied, qkv_bias, qk_norm)
    ...
    while li < @n_layers
      ...
      while hkv < @n_kv
        if @kv_type_k != 8
          TinyNNMetal.upload_row_major(@sess, blk_f.t_K[hkv], kv_zero_k)
        end
        TinyNNMetal.upload_row_major(@sess, blk_f.t_V[hkv], kv_zero_v)
        hkv = hkv + 1
      end
      li = li + 1
    end
  ...
  def build_block_step(t_x, blk, t_pos, pos, scale, eps,
    ...
    while hkv < @n_kv
      ...
      t_v_row = TinyNNMetal.tnn_view_2d(@sess, t_v_new, 1, @d_head, 4, 0)
      t_cpy_v = TinyNNMetal.tnn_cpy(@sess, t_v_row, t_V_slot)
  ...
  def build_attention_qhead_step(t_h, blk, hq, t_pos, pos, scale,
    ...
    if @use_flash_attn
      ...
      t_V_3d = TinyNNMetal.tnn_reshape_3d(@sess, t_V_T, @d_head, hist_count, 1)
    ...
    t_head = TinyNNMetal.tnn_matmul(@sess, t_V_hist, t_attn)
    if tap_this_head
...
module SmolLM2KVMetal
  ...
  def self.upload_from(kv_cache, model)
    ...
    TinyNNMetal.upload_row_major(sess, kv_cache.t_token_embed, model.token_embed.weight)
    TinyNNMetal.tnn_upload_from_float_array(sess, kv_cache.t_final_norm_gamma,
                                        model.final_norm.gamma, d_model)
    if kv_cache.has_untied_output
      TinyNNMetal.upload_row_major(sess, kv_cache.t_output, model.output_proj)
    end
    ...
    while li < n
      ...
      TinyNNMetal.tnn_upload_from_float_array(sess, blk_f.t_rn1_gamma, blk_n.rn1.gamma, d_model)
      TinyNNMetal.tnn_upload_from_float_array(sess, blk_f.t_rn2_gamma, blk_n.rn2.gamma, d_model)

      hq = 0
      while hq < n_heads
        TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_q[hq], blk_n.attn.w_q[hq])
        if kv_cache.has_qkv_bias
          TinyNNMetal.tnn_upload_from_float_array(sess, blk_f.t_b_q[hq], blk_n.attn.b_q[hq], d_head)
        end
        hq = hq + 1
      end
      ...
      hkv = 0
      while hkv < n_kv
        TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_k[hkv], blk_n.attn.w_k[hkv])
        TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_v[hkv], blk_n.attn.w_v[hkv])
        if kv_cache.has_qkv_bias
          TinyNNMetal.tnn_upload_from_float_array(sess, blk_f.t_b_k[hkv], blk_n.attn.b_k[hkv], d_head)
          TinyNNMetal.tnn_upload_from_float_array(sess, blk_f.t_b_v[hkv], blk_n.attn.b_v[hkv], d_head)
        end
      ...
      TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_o,    blk_n.attn.w_o)
      TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_gate, blk_n.ffn.w_gate)
      TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_up,   blk_n.ffn.w_up)
      TinyNNMetal.stage_transposed_and_upload(sess, blk_f.t_w_down, blk_n.ffn.w_down)
