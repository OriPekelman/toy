...
class LlamaSeqForwardFFICache
  ...
  def realize_for_q8_copy(gguf_handle, cfg, t_seq, untied, qkv_bias)
    ...
    @t_seq_token_embed = TinyNN.tnn_input_2d_persistent_typed(@sess,
    ...
    if untied
      ...
      @t_seq_output = TinyNN.tnn_input_2d_persistent_typed(@sess,
    ...
    while li < @seq_n_layers
      ...
      blk.t_seq_w_q = [TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, q_type)]
      hq = 1
      while hq < @seq_n_heads
        blk.t_seq_w_q.push(TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, q_type))
        hq = hq + 1
      end
      ...
      blk.t_seq_w_k = [TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, k_type)]
      blk.t_seq_w_v = [TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, v_type)]
      hkv = 1
      while hkv < @seq_n_kv
        blk.t_seq_w_k.push(TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, k_type))
        blk.t_seq_w_v.push(TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_head, @seq_d_model, v_type))
        hkv = hkv + 1
      end
      ...
      blk.t_seq_w_o    = TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_model, @seq_d_model,
                           TinyNN.tnn_gguf_tensor_type(gguf_handle, o_idx))
      blk.t_seq_w_gate = TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_ff, @seq_d_model,
                           TinyNN.tnn_gguf_tensor_type(gguf_handle, gate_idx))
      blk.t_seq_w_up   = TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_ff, @seq_d_model,
                           TinyNN.tnn_gguf_tensor_type(gguf_handle, up_idx))
      blk.t_seq_w_down = TinyNN.tnn_input_2d_persistent_typed(@sess, @seq_d_model, @seq_d_ff,
    ...
    TinyNN.tnn_gguf_copy_1d_to_persistent(gguf_handle, fnidx, @sess, @t_seq_final_norm_gamma)
    if untied
      oidx2 = TinyNN.tnn_gguf_find_index(gguf_handle, "output.weight")
      TinyNN.tnn_gguf_copy_verbatim_to_persistent(gguf_handle, oidx2, @sess, @t_seq_output)
    end
  ...
  def build_seq_block(t_x, blk, scale, eps)
    ...
    while hkv < @seq_n_kv
      ...
      # no-op (1 == 1).
      tb = @seq_t * @seq_b
      t_k_pre3 = TinyNN.tnn_reshape_3d(@sess, t_k_pre, @seq_d_head, 1, tb)
  ...
  def build_seq_qhead(t_h, blk, hq, t_k_per_kv, t_vt_per_kv, scale)
    ...
    tb = @seq_t * @seq_b
    t_q_pre3 = TinyNN.tnn_reshape_3d(@sess, t_q_pre, @seq_d_head, 1, tb)
    t_q3     = TinyNN.tnn_rope_ext(@sess, t_q_pre3, @t_seq_positions,
                                     ...
                                     @seq_rope_scaling.beta_slow,
                                     @t_seq_rope_freq_factors)
    t_q      = TinyNN.tnn_reshape_2d(@sess, t_q3, @seq_d_head, tb)
    ...
    if @seq_b > 1
      ...
      t_attn = TinyNN.tnn_soft_max_ext(@sess, t_scores, @t_seq_attn_mask, scale, 0.0)
    else
