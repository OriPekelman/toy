# GPT2KVFFICacheMetal — per-step decode with persistent K/V kv_cache.
#
# At each decode step:
...
class GPT2KVFFICacheMetal
  ...
  def initialize
    ...
    @t_pos_embed   = TinyNNMetal.tnn_null_ptr
    @t_ln_f_gamma  = TinyNNMetal.tnn_null_ptr
    @t_ln_f_beta   = TinyNNMetal.tnn_null_ptr
    @kv_blocks_ffi = [GPT2KVBlockFFIMetal.new]
  end
  ...
  def realize_for(max_T, d_model, d_ff, n_heads, n_layers,
    ...
    @t_pos_embed   = TinyNNMetal.tnn_input_2d_f32_persistent(@sess, context_length, d_model)
    @t_ln_f_gamma  = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
    @t_ln_f_beta   = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)

    @kv_blocks_ffi = [GPT2KVBlockFFIMetal.new]
    li = 1
    while li < n_layers
      @kv_blocks_ffi.push(GPT2KVBlockFFIMetal.new)
      li = li + 1
    end
    ...
    while li < n_layers
      ...
      blk.t_ln1_gamma = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
      blk.t_ln1_beta  = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
      blk.t_ln2_gamma = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
      blk.t_ln2_beta  = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)

      # Per-head: weights, biases, and KV buffers.
      blk.t_w_q = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model)]
      blk.t_w_k = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model)]
      blk.t_w_v = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model)]
      ...
      blk.t_b_v = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, 1)]   # ne=[1, d_head]
      # K: ne=[d_head, max_T]; V: ne=[max_T, d_head] (transposed layout).
      blk.t_K   = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, max_T,  d_head)]
      blk.t_V   = [TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, max_T)]
      h = 1
      while h < n_heads
        blk.t_w_q.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model))
        blk.t_w_k.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model))
        blk.t_w_v.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, d_model))
        ...
        blk.t_b_v.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, 1))
        blk.t_K.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, max_T,  d_head))
        blk.t_V.push(TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_head, max_T))
        h = h + 1
      end

      blk.t_w_o   = TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_model, d_model)
      blk.t_b_o   = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
      blk.t_w_ff1 = TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_ff,    d_model)
      blk.t_b_ff1 = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_ff)
      blk.t_w_ff2 = TinyNNMetal.tnn_input_2d_f32_persistent(@sess, d_model, d_ff)
      blk.t_b_ff2 = TinyNNMetal.tnn_input_1d_f32_persistent(@sess, d_model)
      li = li + 1
    end
  ...
  def build_decode_step(pos)
    ...
    t_embed_row = TinyNNMetal.tnn_get_rows(@sess, @t_token_embed, t_token_id)  # ne=[d_model, 1]
    t_pos_row   = TinyNNMetal.tnn_view_2d(@sess, @t_pos_embed,
                                      d_model, 1, bytes_d_model,
                                      pos * bytes_d_model)
    t_x = TinyNNMetal.tnn_add(@sess, t_embed_row, t_pos_row)
    ...
    t_x_final = TinyNNMetal.tnn_layer_norm(@sess, t_x, @t_ln_f_gamma, @t_ln_f_beta, eps)
    t_kv_logits = TinyNNMetal.tnn_matmul(@sess, @t_token_embed, t_x_final)  # ne=[vocab, 1]
    TinyNNMetal.tnn_set_output(t_kv_logits)
    GPT2KVStepResultMetal.new(t_token_id, t_kv_logits)
  end

  def build_block_step(t_x, blk, pos, scale, eps, bytes_d_head, bytes_max_T)
    t_h = TinyNNMetal.tnn_layer_norm(@sess, t_x, blk.t_ln1_gamma, blk.t_ln1_beta, eps)
    ...
    h = 1
    while h < @n_heads
      t_concat = TinyNNMetal.tnn_concat(@sess, t_concat, t_head_outs[h], 0)
      h = h + 1
    end

    t_out_proj_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_o, t_concat)
    t_out_proj     = TinyNNMetal.tnn_add(@sess, t_out_proj_raw, blk.t_b_o)
    t_x_attn       = TinyNNMetal.tnn_add(@sess, t_x, t_out_proj)

    t_h2     = TinyNNMetal.tnn_layer_norm(@sess, t_x_attn, blk.t_ln2_gamma, blk.t_ln2_beta, eps)
    t_up_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_ff1, t_h2)
    t_up     = TinyNNMetal.tnn_add(@sess, t_up_raw, blk.t_b_ff1)
    t_g      = TinyNNMetal.tnn_gelu(@sess, t_up)
    t_dn_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_ff2, t_g)
    t_dn     = TinyNNMetal.tnn_add(@sess, t_dn_raw, blk.t_b_ff2)

    TinyNNMetal.tnn_add(@sess, t_x_attn, t_dn)
  end

  def build_attention_head_step(t_h, blk, head_idx, pos, scale,
                                 bytes_d_head, bytes_max_T)
    # q_new, k_new, v_new for the single new position.
    t_q_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_q[head_idx], t_h)   # ne=[d_head, 1]
    t_q     = TinyNNMetal.tnn_add(@sess, t_q_raw, blk.t_b_q[head_idx])
    t_k_raw = TinyNNMetal.tnn_matmul(@sess, blk.t_w_k[head_idx], t_h)
    t_k_new = TinyNNMetal.tnn_add(@sess, t_k_raw, blk.t_b_k[head_idx])
    t_v_raw = TinyNNMetal.tnn_matmul(@sess, t_h, blk.t_w_v[head_idx])   # ne=[1, d_head]
    t_v_new = TinyNNMetal.tnn_add(@sess, t_v_raw, blk.t_b_v[head_idx])

    # Write k_new → K[pos], v_new → V[:, pos] via cpy-into-view.
    t_K_slot = TinyNNMetal.tnn_view_2d(@sess, blk.t_K[head_idx],
                                    @d_head, 1, bytes_d_head, pos * bytes_d_head)
    t_cpy_k  = TinyNNMetal.tnn_cpy(@sess, t_k_new, t_K_slot)
    t_V_slot = TinyNNMetal.tnn_view_2d(@sess, blk.t_V[head_idx],
    ...
    t_K_hist = TinyNNMetal.tnn_view_2d(@sess, blk.t_K[head_idx],
                                    @d_head, pos + 1, bytes_d_head, 0)
    t_V_hist = TinyNNMetal.tnn_view_2d(@sess, blk.t_V[head_idx],
                                    pos + 1, @d_head, bytes_max_T, 0)

    t_scores = TinyNNMetal.tnn_matmul(@sess, t_K_hist, t_q)        # ne=[pos+1, 1]
    ...
    TinyNNMetal.tnn_matmul(@sess, t_V_hist, t_attn)                 # ne=[d_head, 1]
  end
end
...
module GPT2KVMetal
  ...
  # realized GPT2KVFFICacheMetal. Counterpart to GPT2FFI.upload_from for
