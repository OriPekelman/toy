...
algorithms live under `lib/toy/llm/` in six layers (L6 — experiments/sweeps — lives in Tao, not here); each unit is one
...
lib/toy/llm/
  ...
  engine/       L4  realized session        llama_seq_engine, llama_kv_engine,
                    (graph+weights+driver)  gpt2_seq_engine, vit_tiny_engine
  recipes/      L5  training plan           from_scratch, lora, warm_start, vit_tiny
```
...
decode is the separate `lib/toy/llm/engine/llama_kv_engine.rb` path, out of scope for
this layer).
...
The L5 recipes (from_scratch, lora, warm_start) are mirrored like every
other layer — their `_cuda`/`_metal` siblings are generated by
`prep/gen_cuda_mirror.rb` (toy#65 item 5). `vit_tiny.rb` is CPU-only.

---

## Mirror discipline

Backends are CPU (default), CUDA (GB10), and Metal. Each mirrored layer
(L1–L5; see `MIRRORABLE` for the exact list) carries a generated `_cuda`
and `_metal` sibling. **You only write the CPU file.**

The generator is `prep/gen_cuda_mirror.rb`. The set of files it mirrors
is the `MIRRORABLE` list at the top of that script; the L1-L5 algorithm
...
session backend id, and rewrites any `require_relative ".../ffi/tinynn"`.
...
The L1 files deliberately omitting the TinyNN require (above) is
