...
- [`consuming-toy.md`](consuming-toy.md) — **how to use toy as a
  vendored gem from a downstream research project** (e.g.
  `tao_transfer`). End-to-end workflow: Gemfile → `bundle lock` →
  `spinel-compat vendor` → `prep/post_vendor_toy.rb` → `spinel
  experiment.rb`. Pairs with the spinelgems convention. Current
  reference; the toy#19 commit it documents shipped 2026-05-27.
- [`coverage.md`](coverage.md) — **the canonical ggml-op coverage
  matrix**. Auto-generated by `prep/gen_coverage.rb` from the
  source tree. One row per ggml op with CPU/CUDA/Metal binding
  status + backward. Run `make coverage` to regenerate; `make
  coverage-check` for CI.
- [`events-schema.md`](events-schema.md) — the JSON Lines event
  stream Toy emits during training and inference. Frozen v1
  contract. The substrate for Tao (`../tao/`), live TUIs, post-run
  analysis, W&B / MLflow adapters.
...
- [`notes/mul_mat_id_quants.md`](notes/mul_mat_id_quants.md) —
  ggml's `mul_mat_id` produces wrong output on K-quantized
  (Q4_K / Q5_K / Q6_K) expert weights. Filed as
  [ggml-org/ggml#1506](https://github.com/ggml-org/ggml/issues/1506).
  Workaround: use Q8_0 for MoE expert weights.
