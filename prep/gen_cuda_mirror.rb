...
MIRRORABLE = [
  ...
  "lib/toy/llm/primitives/mla.rb",      # K-series M2: L1 Gated-MLA pieces (calls TinyNN.* + GQA/RMSNorm)
  ...
  "lib/toy/llm/engine/gtx_engine.rb",    # tao#24: graph-transformer engine — the CUDA twin exists for `--task bytelm` ONLY
  "lib/toy/llm/recipes/gtx_graph.rb",    # tao#24: L4 recipe over GtxEngine
  "lib/toy/run/train_gtx.rb",            # tao#24: the gtx runner is MECHANICALLY mirrorable, so the twin cannot drift
...
def subs_for(cpu_path, backend)
  ...
  when "lib/toy/llm/primitives/mla.rb"
    # K-series M2 primitive. It reaches Primitives::GQA.attention and
    # Primitives::RMSNorm.build; those module names collide with nothing
    # per-backend, so only the TinyNN. rewrite in common_module_tail
    # applies — same shape as the kda/muon rules.
    [
      [/^# lib\/toy\/llm\/primitives\/mla\.rb.*$/,
       "# lib/toy/llm/primitives/mla_#{backend}.rb — #{label} mirror of lib/toy/llm/primitives/mla.rb.\n" +
       "#\n" +
       "# AUTO-GENERATED by prep/gen_cuda_mirror.rb. Do not edit by hand;\n" +
       "# edit the CPU source and re-run the generator. Same L1 contract\n" +
       "# on the GPU backend via #{module_}."],
  ...
  when "lib/toy/llm/engine/gtx_engine.rb"
    # tao#24 — the graph-transformer engine (class Toy::LLM::Engine::GtxEngine).
    # Its only require_relatives are ../../models/transformer (which owns Mat
    # and pulls in the CPU TinyNN) and ../../train/dfa_b (pure Ruby, backend-
    # neutral): NEITHER is rewritten, because the mirror still wants Mat and
    # DfaB from exactly those files. The backend-sensitive content is the class
    # name plus every TinyNN.* call and tnn_session_new(0) — common_module_tail.
    #
    # SCOPE: the CUDA twin is built for `--task bytelm` only (tao#24). The
    # relational task compiles into the mirror because the file is one unit,
    # but the runner refuses it and the CLI never selects it.
    [
      [/^# lib\/toy\/llm\/engine\/gtx_engine\.rb.*$/,
       "# lib/toy/llm/engine/gtx_engine_#{backend}.rb — #{label} mirror of lib/toy/llm/engine/gtx_engine.rb.\n" +
       "#\n" +
       "# AUTO-GENERATED by prep/gen_cuda_mirror.rb. Do not edit by hand;\n" +
       "# edit the CPU source and re-run the generator. The CPU file's\n" +
       "# header explains the architecture; this mirror keeps the same\n" +
       "# contract on the GPU backend via #{module_} (tao#24: the twin\n" +
       "# exists for the byte-LM task only)."],
      [/\bGtxEngine\b/, "GtxEngine" + suffix],
    ] + common_module_tail

  when "lib/toy/llm/recipes/gtx_graph.rb"
    # tao#24 — L4 recipe over GtxEngine. Same shape as from_scratch: rename
    # the recipe class + the engine class it instantiates; the inlined
    # TinyNN.* calls in step!/step_bytelm! come from common_module_tail.
    # No require of its own (the runner loads the engine first).
    [
      [/^# lib\/toy\/llm\/recipes\/gtx_graph\.rb.*$/,
       "# lib/toy/llm/recipes/gtx_graph_#{backend}.rb — #{label} mirror of lib/toy/llm/recipes/gtx_graph.rb.\n" +
       "#\n" +
       "# AUTO-GENERATED by prep/gen_cuda_mirror.rb. Do not edit by hand;\n" +
       "# edit the CPU source and re-run the generator. Same L4 contract\n" +
       "# on the GPU backend via #{module_}."],
      [/\bGtxEngine\b/, "GtxEngine" + suffix],
      [/\bGtxGraph\b/,  "GtxGraph"  + suffix],
    ] + common_module_tail

  when "lib/toy/run/train_gtx.rb"
    # tao#24 — THE RUNNER, and it is in MIRRORABLE on purpose.
    #
    # Every other GPU runner in this tree is hand-written, for a good
    # reason each time (a ctor arity that differs, a monomorphic mmap cfg).
    # train_gtx.rb has neither: the ONLY backend-sensitive content is the
    # TinyNN.* namespace, tnn_session_new(0), the engine/recipe class names
    # and the two require paths. Hand-copying 980 lines to get that would
    # buy a drift surface for nothing — and this lane's deliverable is
    # CROSS-BACKEND NUMERIC AGREEMENT, which is only meaningful if both
    # devices run the same program. Mechanically mirrored, they provably do.
    #
    # The require of ../ffi/tinynn is rewritten to tinynn_cuda so the mirror
    # loads the GPU shim; the CPU TinyNN module still arrives transitively
    # (gtx_engine_<backend> -> ../../models/transformer -> ../ffi/tinynn),
    # which is what Mat and ToyDescribeFlow's TinyNN.* introspection need.
    # Same both-shims-in-one-unit shape as libexec/toy-train-cuda.
    [
      # NOTE the substitution is LINE-WISE, and the CPU header's first
      # sentence wraps onto line 2. So the banner is PREPENDED and the
      # original line 1 is kept verbatim underneath it — line 2 then still
      # reads as its continuation instead of dangling.
      [/^# lib\/toy\/run\/train_gtx\.rb.*$/,
       "# lib/toy/run/train_gtx_#{backend}.rb — #{label} mirror of\n" +
       "# lib/toy/run/train_gtx.rb (-> libexec/toy-train-gtx-#{backend}), the\n" +
       "# tao#24 byte-LM twin.\n" +
       "#\n" +
       "# AUTO-GENERATED by prep/gen_cuda_mirror.rb. Do not edit by hand;\n" +
       "# edit the CPU source and re-run the generator. This twin REFUSES\n" +
       "# every task but `bytelm` (tao#24 scope) — see the guard below.\n" +
       "#\n" +
       "# The CPU source's own header follows, verbatim.\n" +
       "#\n" +
       "# lib/toy/run/train_gtx.rb — Spinel-compiled graph-transformer training"],
      # The CPU runner has NO tinynn require of its own (it arrives
      # transitively). The mirror needs the GPU shim explicitly, so it is
      # INSERTED here rather than added to the CPU source — the CPU file's
      # require list stays byte-identical, which is the whole point of
      # "a default that changes nothing".
      [/^require_relative "\.\.\/llm\/engine\/gtx_engine"$/,
       'require_relative "../ffi/tinynn_' + backend.to_s + "\"\n" +
       'require_relative "../llm/engine/gtx_engine_' + backend.to_s + '"'],
      [/^require_relative "\.\.\/llm\/recipes\/gtx_graph"$/,
       'require_relative "../llm/recipes/gtx_graph_' + backend.to_s + '"'],
      [/\bGtxEngine\b/, "GtxEngine" + suffix],
      [/\bGtxGraph\b/,  "GtxGraph"  + suffix],
      [/toy-train-gtx:/, "toy-train-gtx-" + backend.to_s + ":"],
    ] + common_module_tail
...
def derive_mirror_path(cpu_path, backend)
  ...
  unless cpu_path =~ /_ffi(_[a-z]+)?\.rb$/ ||
         ...
         # tao#24 — runners are hand-written by default (a differing ctor
         # arity or a monomorphic mmap cfg makes them non-mechanical). This
         # allows the exceptions that ARE mechanical; MIRRORABLE, not this
         # regex, is what decides which runners qualify.
         cpu_path =~ %r{^lib/toy/run/[a-z0-9_]+\.rb$}
