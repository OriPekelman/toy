...
const FACETS = [
  {
    key: 'arch-forward',
    prompt: `Map the ORCHESTRATION to be lifted: LlamaSeqForwardFFICache#build_forward_in_current_ctx (around lines 1532-1582) in ${MONOLITH}. Detail every step: token_ids/positions input build, the per-block TransformerBlockCtx construction, the get_rows embed, the optional projection-lens branch (donor_d_in), the block-stacking while-loop calling blk.build_forward, the final RMSNorm.build, the tied/untied LM-head matmul, and every tnn_set_output tap. List in ivars_read the config/handle ivars it reads, and in cache_side_effects every @t_seq_* ivar it WRITES (x_embed, x_final, logits, token_ids, positions, ...) — find where each of those is read elsewhere (grep), because the arch API must keep them visible.`,
  },
  {
    key: 'arch-weights',
